...
class FlaxBertEmbeddings(nn.Module):
    ...
    def setup(self):
        ...
        self.LayerNorm = ours.LayerNorm(epsilon=self.config.layer_norm_eps, dtype=self.dtype)
        self.dropout = ours.Dropout(rate=self.config.hidden_dropout_prob)
        self.add1 = ours.Add()
        self.add2 = ours.Add()
    ...
    def relprop(self, cam, input_ids, token_type_ids, position_ids, attention_mask, deterministic: bool = True):
        inputs_embeds = self.word_embeddings(input_ids.astype("i4"))
        position_embeds = self.position_embeddings(position_ids.astype("i4"))
        token_type_embeddings = self.token_type_embeddings(token_type_ids.astype("i4"))
...
class FlaxBertSelfAttention(nn.Module):
    ...
    def __call__(
        ...
        if layer_head_mask is not None:
            attn_weights = self.einsum1(attn_weights, layer_head_mask)

        attn_output = self.einsum2(attn_weights, value_states)
    ...
    def relprop(self, cam, hidden_states,
        ...
        # get query proj
        query_states_merged = self.query(hidden_states)
        # get key, value proj
        if is_cross_attention:
            # cross_attentions
            key_states_merged = self.key(key_value_states)
            value_states_merged = self.value(key_value_states)
        else:
            # self_attention
            key_states_merged = self.key(hidden_states)
            value_states_merged = self.value(hidden_states)

        query_states = self._split_heads(query_states_merged)
        key_states = self._split_heads(key_states_merged)
        value_states = self._split_heads(value_states_merged)
        ...
        # Mask heads if we want to
        if layer_head_mask is not None:
            attn_weights_masked = self.einsum1(attn_weights, layer_head_mask)
        else:
            attn_weights_masked = attn_weights

        attn_output = self.einsum2(attn_weights_masked, value_states)
        attn_output = attn_output.reshape(attn_output.shape[:2] + (-1,))
        ...
        cam1_1 = self._merge_heads(cam1_1)
        ...
        if is_cross_attention:
            # cross_attentions
            cam1_2 = self.key.relprop(cam1_2, key_value_states)
            cam2 = self.value.relprop(cam2, key_value_states)
        else:
            # self_attention
